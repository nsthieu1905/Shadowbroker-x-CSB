...
import { AlertTriangle, Clock, ChevronDown, ChevronUp, Newspaper } from "lucide-react";
...
const AIRCRAFT_WIKI: Record<string, string> = {
  ...
  A21N: "Airbus A321neo",
  // Embraer
  E135: "Embraer ERJ 145 family",
  E145: "Embraer ERJ 145 family",
  ...
  F18H: "Boeing F/A-18E/F Super Hornet",
...
function NewsFeedInner({
  ...
  const {
    imgUrl: aircraftImgUrl,
    wikiUrl: aircraftWikiUrl,
    loading: aircraftImgLoading,
  } = useAircraftImage(selectedFlightModel);
    ...
    return (
      ...
      >
        <div className="p-3 border-b border-emerald-500/30 bg-emerald-950/40 flex justify-between items-center">
          <h2 className="text-xs tracking-widest font-bold text-emerald-400">HỒ SƠ KHU VỰC</h2>
        ...
        {regionDossierLoading ? (
          <div className="p-6 flex items-center justify-center">
            <span className="text-emerald-400 text-[10px] font-mono animate-pulse tracking-widest">
          ...
          <div className="p-3 flex flex-col gap-1.5 max-h-[500px] overflow-y-auto styled-scrollbar text-[10px]">
            {/* COUNTRY */}
            <div className="text-[9px] text-emerald-500 tracking-widest font-bold border-b border-emerald-900/50 pb-1">
              CẤP QUỐC GIA {d.country?.flag_emoji || ""}
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">QUỐC GIA</span>
              <span className="text-[var(--text-primary)] font-bold">{d.country?.name}</span>
            </div>
            {d.country?.official_name && d.country.official_name !== d.country.name && (
              <div className="flex justify-between">
                <span className="text-[var(--text-muted)]">TÊN CHÍNH THỨC</span>
                <span className="text-[var(--text-secondary)] text-right max-w-[180px]">{d.country.official_name}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">LÃNH ĐẠO</span>
              <span className="text-emerald-400 font-bold">{d.country?.leader}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">CHÍNH THỂ</span>
              <span className="text-[var(--text-primary)] font-bold text-right max-w-[180px]">
                {d.country?.government_type}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">DÂN SỐ</span>
              <span className="text-[var(--text-primary)] font-bold">{d.country?.population?.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">THỦ ĐÔ</span>
              <span className="text-[var(--text-primary)] font-bold">{d.country?.capital}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">NGÔN NGỮ</span>
              <span className="text-[var(--text-primary)] text-right max-w-[180px]">
                {d.country?.languages?.join(", ")}
              </span>
            </div>
            {d.country?.currencies?.length > 0 && (
              <div className="flex justify-between">
                <span className="text-[var(--text-muted)]">TIỀN TỆ</span>
                <span className="text-[var(--text-primary)] text-right max-w-[180px]">
                  {d.country.currencies.join(", ")}
                </span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">KHU VỰC</span>
              <span className="text-[var(--text-primary)]">{d.country?.subregion || d.country?.region}</span>
            </div>
            {d.country?.area_km2 > 0 && (
              <div className="flex justify-between">
                <span className="text-[var(--text-muted)]">DIỆN TÍCH</span>
                <span className="text-[var(--text-primary)]">{d.country.area_km2.toLocaleString()} km²</span>
              </div>
            )}
            ...
            {(d.local?.name || d.local?.state) && (
              <>
                <div className="text-[9px] text-emerald-500 tracking-widest font-bold border-b border-emerald-900/50 pb-1 mt-2">
                  CẤP ĐỊA PHƯƠNG
                </div>
                {d.local.island && (
                  <div className="flex justify-between">
                    <span className="text-[var(--text-muted)]">ĐẢO/QUẦN ĐẢO</span>
                    <span className="text-[var(--text-primary)] font-bold text-right max-w-[180px]">
                      {d.local.island}
                    </span>
                  </div>
                ...
                {d.local.name && (
                  <div className="flex justify-between">
                    <span className="text-[var(--text-muted)]">ĐỊA PHƯƠNG</span>
                    <span className="text-[var(--text-primary)] font-bold">{d.local.name}</span>
                  </div>
                )}
                {d.local.state && (
                  <div className="flex justify-between">
                    <span className="text-[var(--text-muted)]">TỈNH/THÀNH</span>
                    <span className="text-[var(--text-primary)] font-bold">{d.local.state}</span>
                  </div>
                )}
                {d.local.description && (
                  <div className="flex justify-between">
                    <span className="text-[var(--text-muted)]">LOẠI</span>
                    <span className="text-[var(--text-secondary)]">{d.local.description}</span>
                  </div>
                )}
                {d.local.summary && (
  ...
  if (selectedEntity?.type === "tracked_flight") {
    ...
    if (flight) {
      ...
      const alertBorderMap: Record<string, string> = {
        "#ff1493": "border-[#ff1493]/30",
        pink: "border-[#ff1493]/30",
        red: "border-red-500/30",
        yellow: "border-yellow-500/30",
        blue: "border-blue-500/30",
        orange: "border-orange-500/30",
        "#32cd32": "border-[#32cd32]/30",
        purple: "border-purple-500/30",
        black: "border-gray-500/30",
        white: "border-[var(--border-primary)]/30",
      };
      const alertBgMap: Record<string, string> = {
        ...
        black: "bg-gray-900/40",
        white: "bg-[var(--bg-panel)]",
      };
          ...
          <div className="p-4 flex flex-col gap-3">
            ...
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              ...
              <span className="text-[var(--text-primary)] text-xs font-bold">
                {flight.alert_type || flight.model || "UNKNOWN"}
              </span>
            </div>
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
            ...
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              ...
              <span className="text-[var(--text-primary)] text-xs font-bold">
                {Math.round((flight.alt || 0) / 0.3048).toLocaleString()} ft
              </span>
            </div>
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
            ...
            {flight.squawk && (
              <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                ...
                <span
                  className={`text-xs font-bold ${flight.squawk === "7700" ? "text-red-400 animate-pulse" : flight.squawk === "7600" ? "text-yellow-400" : "text-[var(--text-primary)]"}`}
                >
                  {flight.squawk}
                  {flight.squawk === "7700" ? " ⚠ EMERGENCY" : flight.squawk === "7600" ? " COMMS LOST" : ""}
                </span>
              </div>
            )}
            {flight.alert_link && (
              <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                ...
                <a
                  href={flight.alert_link}
                  target="_blank"
                  rel="noreferrer"
                  className={`text-xs font-bold underline ${headerColor} hover:opacity-80`}
                >
                  View Intel Source
                </a>
              </div>
      ...
      return (
        ...
        >
          <div className="p-3 border-b border-[var(--border-primary)]/30 bg-[var(--bg-secondary)]/40 flex justify-between items-center">
            <h2
              className={`text-xs tracking-widest font-bold ${selectedEntity.type === "military_flight" ? "text-red-400" : selectedEntity.type === "private_flight" ? "text-orange-400" : selectedEntity.type === "private_jet" ? "text-purple-400" : "text-cyan-400"} flex items-center gap-2`}
            >
              {selectedEntity.type === "military_flight"
                ? "MILITARY BOGEY INTERCEPT"
                : selectedEntity.type === "private_flight"
                  ? "PRIVATE TRANSPONDER"
                  : selectedEntity.type === "private_jet"
                    ? "PRIVATE JET TRANSPONDER"
          ...
          <div className="p-4 flex flex-col gap-3">
            ...
            {(aircraftImgUrl || aircraftImgLoading || aircraftWikiUrl) && (
              <div className="border-b border-[var(--border-primary)] pb-3">
                {aircraftImgLoading && <div className="w-full h-24 rounded bg-[var(--bg-tertiary)]/60 animate-pulse" />}
                {aircraftImgUrl && (
                  <a href={aircraftWikiUrl || "#"} target="_blank" rel="noopener noreferrer" className="block">
                    <img
                      src={aircraftImgUrl}
            ...
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              ...
              <span className="text-cyan-400 text-xs font-bold">
                {flight.origin_name !== "UNKNOWN" ? `[${flight.origin_name}] → [${flight.dest_name}]` : "UNKNOWN"}
              </span>
            </div>
            {flight.icao24 && (
              <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
        ...
        >
          ...
          <div className="p-4 flex flex-col gap-3">
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              ...
              <span className="text-[var(--text-primary)] text-xs font-bold text-right ml-4">
                {ship.name || "UNKNOWN"}
              </span>
            </div>
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
            ...
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              ...
              <span
                className={`text-xs font-bold ${ship.destination && ship.destination !== "UNKNOWN" ? "text-cyan-400" : "text-orange-400"}`}
              >
            ...
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              ...
              <span className="text-[var(--text-primary)] text-xs font-bold">
                {ship.type === "carrier" ? "UNKNOWN" : `${ship.sog || 0} kts`}
              </span>
            </div>
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              <span className="text-[var(--text-muted)] text-[10px]">COURSE (COG)</span>
              <span className="text-[var(--text-primary)] text-xs font-bold">
                {ship.type === "carrier" ? "UNKNOWN" : `${Math.round(ship.cog || 0)}°`}
              </span>
            </div>
            {ship.mmsi && (
              <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
  ...
  if (selectedEntity?.type === "gdelt") {
    const gdeltItem = data?.gdelt?.find(
            ...
            <div className="flex flex-col gap-2 mt-2">
              ...
              <div className="flex flex-col gap-1 max-h-[250px] overflow-y-auto styled-scrollbar">
                {(() => {
                  ...
                  if (urls.length === 0)
                    return <span className="text-[var(--text-muted)] text-[10px]">No articles available.</span>;
            ...
            {item.link && (
              <div className="flex justify-between items-center pb-2 mt-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-yellow-400 hover:text-yellow-300 text-xs font-bold underline"
                >
        ...
        >
          ...
          <div className="p-4 flex flex-col gap-3">
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              ...
              <span className="text-[var(--text-primary)] text-[10px] font-bold text-right ml-4 break-words">
                {apt.name}
              </span>
            </div>
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
      ...
      >
        <div className="p-3 border-b border-[var(--border-primary)]/30 bg-[var(--bg-secondary)]/40 flex justify-between items-center">
          <h2 className="text-xs tracking-widest font-bold text-cyan-400 flex items-center gap-2">
            ...
            {selectedEntity.extra?.last_updated
              ? new Date(selectedEntity.extra.last_updated + "Z")
        ...
        <div className="relative w-full h-48 bg-black flex items-center justify-center p-1">
          {(() => {
            ...
            const mt =
              selectedEntity.extra?.media_type ||
              (url.includes(".mp4") || url.includes(".webm")
                ? "video"
                : url.includes(".m3u8") || url.includes("hls")
                  ? "hls"
                  : url.includes(".mjpg") || url.includes(".mjpeg") || url.includes("mjpg")
                    ? "mjpeg"
                    : url.includes("embed") || url.includes("maps/embed")
                      ? "embed"
        ...
        <div className="p-3 bg-black/40 text-[9px] text-cyan-500/70 font-mono tracking-widest flex justify-between items-center">
          ...
          <span className="text-red-500 text-right">
            {selectedEntity.extra?.last_updated
              ? new Date(selectedEntity.extra.last_updated + "Z").toLocaleString("en-US", {
                  month: "short",
                  day: "numeric",
                  hour: "2-digit",
                  minute: "2-digit",
          ...
          {!isMinimized && (
            ...
            >
              <span className="flex items-center gap-1">
                <Clock size={10} /> {data?.last_updated ? formatTime(data.last_updated) : "SCANNING"}
              </span>
            </motion.div>
          )}
        </AnimatePresence>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between text-[8px] text-[var(--text-secondary)] uppercase tracking-widest">
                    <span className="font-bold flex items-center gap-1 text-cyan-600">&gt;_ {item.source}</span>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className={`text-[11px] ${titleClass} hover:text-[var(--text-primary)] transition-colors leading-tight`}
                  >
                    {item.title}
                  </a>
                    ...
                    <div className="flex items-center gap-2">
                      {item.cluster_count > 1 && (
                        <button
                          ...
                          className="text-[8px] font-bold text-cyan-500 bg-[var(--bg-secondary)]/50 hover:text-[var(--text-primary)] hover:bg-[var(--hover-accent)] border border-cyan-500/30 px-1.5 py-0.5 rounded-sm transition-colors cursor-pointer"
                        >
                          {isExpanded ? "[Ẩn bớt]" : `[+${item.cluster_count - 1} Xem thêm]`}
                        </button>
                      )}
                      {item.coords && (
                        ...
                        {item.articles.slice(1).map((subItem: any, subIdx: number) => (
                          <div key={subIdx} className="flex flex-col gap-0.5 pl-2 border-l border-cyan-500/20">
                            <div className="flex items-center justify-between text-[7.5px] text-[var(--text-muted)] uppercase font-bold">
                              ...
                              <span
                                className={
                                  subItem.risk_score >= 9
                                    ? "text-red-400"
                                    : subItem.risk_score >= 7
                                      ? "text-orange-400"
                                      : subItem.risk_score >= 4
                                        ? "text-yellow-500"
                                        : "text-green-400"
                                }
                              >
                                LVL: {subItem.risk_score}/10
                              </span>
                            </div>
                            <a
                              href={subItem.link}
                              target="_blank"
                              rel="noreferrer"
                              className="text-[10px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors leading-tight"
                            >
                              {subItem.title}
                            </a>
                          </div>
