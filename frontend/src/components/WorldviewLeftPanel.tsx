...
function relativeTime(iso: string | undefined, nowMs: number): string {
  if (!iso) return "";
  if (!nowMs) return "";
  const diff = nowMs - new Date(iso + "Z").getTime();
...
import type { DashboardData, SelectedEntity, KiwiSDR } from "@/types/dashboard";
...
const WorldviewLeftPanel = React.memo(function WorldviewLeftPanel({
  ...
  const { militaryShipCount, cargoShipCount, passengerShipCount, civilianShipCount, trackedYachtCount } =
    useMemo(() => {
  ...
  const potusFlights = useMemo(() => {
    ...
    if (!tracked) return [];
    const results: { index: number; flight: TrackedFlight; meta: { label: string; type: string } }[] = [];
    for (let i = 0; i < tracked.length; i++) {
      const f = tracked[i];
      const icao = (f.icao24 || "").toUpperCase();
      if (POTUS_ICAOS[icao]) {
        results.push({ index: i, flight: f as TrackedFlight, meta: POTUS_ICAOS[icao] });
      }
    }
    return results;
  ...
  const layers = [
    ...
    {
      id: "ships_military",
      name: "Tàu quân sự / Tàu sân bay",
      source: "AIS Stream",
      count: militaryShipCount,
      icon: Ship,
    },
    { id: "ships_cargo", name: "Tàu hàng / Tàu dầu", source: "AIS Stream", count: cargoShipCount, icon: Ship },
    { id: "ships_civilian", name: "Tàu dân sự", source: "AIS Stream", count: civilianShipCount, icon: Anchor },
    {
      id: "ships_passenger",
      name: "Tàu du lịch / Hành khách",
      source: "AIS Stream",
    ...
    {
      id: "global_incidents",
      name: "Sự kiện toàn cầu",
      source: "GDELT",
      count: data?.gdelt?.length || 0,
      icon: Activity,
    },
    { id: "cctv", name: "Lưới CCTV", source: "CCTV Mesh + Street View", count: data?.cctv?.length || 0, icon: Cctv },
    { id: "gps_jamming", name: "Nhiễu GPS", source: "ADS-B NACp", count: data?.gps_jamming?.length || 0, icon: Radio },
    { id: "gibs_imagery", name: "MODIS Terra (hàng ngày)", source: "NASA GIBS", count: null, icon: Globe },
    {
      id: "highres_satellite",
      name: "Vệ tinh độ phân giải cao",
      source: "Esri World Imagery",
      count: null,
      icon: Satellite,
    },
    { id: "kiwisdr", name: "Trạm thu KiwiSDR", source: "KiwiSDR.com", count: data?.kiwisdr?.length || 0, icon: Radio },
    {
      id: "firms",
      name: "Điểm cháy (24h)",
    ...
    { id: "day_night", name: "Chu kỳ ngày / đêm", source: "Solar Calc", count: null, icon: Sun },
  ];
    ...
    >
      ...
      <div className="mb-6 pointer-events-auto">
        ...
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold tracking-[0.2em] text-[var(--text-heading)]">FLYT</h1>
          <button
            onClick={toggleTheme}
          ...
          <button
            ...
            title={hudColor === "cyan" ? "Chuyển sang HUD kiểu Matrix" : "Chuyển sang HUD màu Cyan"}
          >
            <Palette size={14} />
          </button>
          {onSettingsClick && (
          ...
          {onLegendClick && (
            <button
              ...
              title="Chú giải bản đồ / Ký hiệu"
            >
              <BookOpen size={12} />
              <span className="text-[8px] font-mono tracking-widest font-bold">CHÚ GIẢI</span>
            </button>
          )}
          <span
            className={`h-7 px-2 rounded-lg border border-[var(--border-primary)] flex items-center justify-center text-[8px] ${theme === "dark" ? "text-cyan-400" : "text-[var(--text-muted)]"} font-mono tracking-widest select-none`}
          >
      ...
      <div className="bg-[var(--bg-primary)]/40 backdrop-blur-md border border-[var(--border-primary)] rounded-xl pointer-events-auto shadow-[0_4px_30px_rgba(0,0,0,0.2)] flex flex-col relative overflow-hidden max-h-full">
        {/* Header / Toggle */}
        <div className="flex justify-between items-center p-4 cursor-pointer hover:bg-[var(--bg-secondary)]/50 transition-colors border-b border-[var(--border-primary)]/50">
          <span
            className="text-[10px] text-[var(--text-muted)] font-mono tracking-widest"
            ...
            <button
              ...
              onClick={(e) => {
                ...
                setActiveLayers((prev) => {
                  const next: ActiveLayers = { ...prev };
                  for (const k of Object.keys(prev) as (keyof ActiveLayers)[]) {
                  ...
                  <div className="bg-amber-950/20 border border-amber-500/40 rounded-lg p-3 -mt-1 shadow-[0_0_15px_rgba(245,158,11,0.1)]">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-amber-400 font-mono tracking-widest font-bold">
                          THEO DÕI SDR
                        </span>
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex flex-col p-2 rounded-lg border border-amber-500/20 bg-amber-950/10">
                        <span className="text-[10px] font-bold font-mono text-amber-300 truncate mb-1">
                          {(trackedSdr.name || "TRẠM THU TỪ XA").toUpperCase()}
                        </span>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <button
                            ...
                            title="Di chuyển bản đồ đến vị trí SDR"
                          >
                            <Globe size={10} /> KHÓA LẠI
                          </button>
                          {trackedSdr.url && (
                            <a
                              href={trackedSdr.url}
                  ...
                  <div className="bg-[#ff1493]/5 border border-[#ff1493]/30 rounded-lg p-3 -mt-1">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-[#ff1493] font-mono tracking-widest font-bold">
                          POTUS FLEET
                        </span>
                        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded-full bg-[#ff1493]/20 border border-[#ff1493]/40 text-[#ff1493] animate-pulse">
                          {potusFlights.length} ĐANG HOẠT ĐỘNG
                        </span>
                      </div>
                      <button
                        onClick={(e) => {
                    ...
                    <div className="flex flex-col gap-2">
                      {potusFlights.map((pf) => {
                        const color =
                          pf.meta.type === "AF1" ? "#ff1493" : pf.meta.type === "M1" ? "#ff1493" : "#3b82f6";
                        const alt = pf.flight.alt_baro || pf.flight.alt || 0;
                        const speed = pf.flight.gs || pf.flight.speed || 0;
                        const flightId = pf.flight.icao24 || `${pf.index}`;
                        return (
                          <div
                            key={flightId}
                            ...
                            onClick={() => {
                              ...
                              if (onEntityClick) {
                                onEntityClick({ type: "tracked_flight", id: flightId });
                              }
                            }}
                          >
                            <div className="flex flex-col">
                              ...
                              <span className="text-[8px] text-[var(--text-muted)] font-mono mt-0.5">
                                {alt > 0 ? `${Math.round(alt).toLocaleString()} ft` : "MẶT ĐẤT"} ·{" "}
                                {speed > 0 ? `${Math.round(speed)} kts` : "TĨNH"}
                              </span>
                            </div>
                  ...
                  return (
                    <div key={idx} className="flex flex-col">
                      <div
                        ...
                        onClick={() => setActiveLayers((prev) => ({ ...prev, [layer.id]: !active }))}
                      >
                        <div className="flex gap-3">
                          <div
                            className={`mt-1 ${active ? "text-cyan-400" : "text-gray-600 group-hover:text-gray-400"} transition-colors`}
                          >
                            {layer.id.startsWith("ships_") ? shipIcon : <Icon size={16} strokeWidth={1.5} />}
                          </div>
                          <div className="flex flex-col">
                            <span
                              className={`text-sm font-medium ${active ? "text-[var(--text-primary)]" : "text-[var(--text-secondary)]"} tracking-wide`}
                            >
                              {layer.name}
                            </span>
                            <span className="text-[9px] text-[var(--text-muted)] font-mono tracking-wider mt-0.5">
                              {layer.source} ·{" "}
                              {active
                                ? (() => {
                                    const fKey = FRESHNESS_MAP[layer.id];
                                    const freshness = fKey && data?.freshness?.[fKey];
                                    const rt = freshness ? relativeTime(freshness, nowMs) : "";
                                    return rt ? <span className="text-cyan-500/70">{rt}</span> : "TRỰC TIẾP";
                                  })()
                                : "TẮT"}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div
                            className={`text-[9px] font-mono tracking-wider px-2 py-0.5 rounded-full border ${
                              active
                        ...
                        <div className="ml-7 mt-2 flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1">
                              <span className="text-[8px] text-[var(--text-muted)] font-mono">ĐỘ MỜ</span>
                              <input
                                type="range"
                                min={0}
                                max={100}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {!potusEnabled ? (
                        ...
                        <span className="text-[8px] font-mono text-[var(--text-muted)]">
                          KHÔNG CÓ MÁY BAY ĐANG HOẠT ĐỘNG
                        </span>
                      )}
                    </div>
                  </div>
