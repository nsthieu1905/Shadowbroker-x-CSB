...
export default function FindLocateBar({ data, onLocate, onFilter }: FindLocateBarProps) {
  ...
  const allEntities = useMemo(() => {
    ...
    for (const f of data?.commercial_flights || []) {
      ...
      results.push({
        ...
        sublabel: `${f.model || "Không rõ"} · ${f.airline_code || "Thương mại"}`,
        category: "THƯƠNG MẠI",
    ...
    for (const f of [...(data?.private_flights || []), ...(data?.private_jets || [])]) {
      ...
      results.push({
        ...
        sublabel: `${f.model || "Không rõ"} · Tư nhân`,
        category: "TƯ NHÂN",
    ...
    for (const f of data?.military_flights || []) {
      ...
      results.push({
        id: `mil-flight-${uid}`,
        label: f.callsign || uid,
        sublabel: `${f.model || "Không rõ"} · ${f.military_type || "Quân sự"}`,
        category: "QUÂN SỰ",
    ...
    for (const f of data?.tracked_flights || []) {
      const uid = f.icao24 || f.registration || f.callsign || "";
      const operator = f.alert_operator || "Không rõ đơn vị";
      const category = f.alert_category || "Tracked";
      const type = f.alert_type || f.model || "Không rõ";
    ...
    for (const s of data?.ships || []) {
      results.push({
        ...
        sublabel: `${s.type || "Vessel"} · ${s.destination || "Không rõ đích"}`,
        category: "HÀNG HẢI",
    ...
    for (const op of trackedOperators) {
      results.push({
        id: `tracked-db-${op}`,
        label: op,
        sublabel: `Bản ghi CSDL · Operator`,
        category: "CSDL",
  ...
  const filtered = useMemo(() => {
    ...
    return allEntities
      .filter((e) => {
        const searchable = `${e.label} ${e.sublabel} ${e.id} ${e._extra || ""}`.toLowerCase();
  ...
  const handleSelect = (result: SearchResult) => {
    if (result.entityType === "database_operator") {
      ...
      onLocate(result.lat, result.lng);
    }
    setQuery("");
    setIsOpen(false);
  };
  ...
  const categoryIcons: Record<string, React.ReactNode> = {
    "THƯƠNG MẠI": <Plane size={10} className="text-cyan-400" />,
    "TƯ NHÂN": <Plane size={10} className="text-orange-400" />,
    "QUÂN SỰ": <Shield size={10} className="text-yellow-400" />,
    "THEO DÕI": <Star size={10} className="text-pink-400" />,
    "HÀNG HẢI": <Ship size={10} className="text-blue-400" />,
    CSDL: <Database size={10} className="text-purple-400" />,
  };
    ...
    <div ref={containerRef} className="relative w-full pointer-events-auto">
      <div className="flex items-center gap-2 bg-[var(--bg-primary)]/40 backdrop-blur-md border border-[var(--border-primary)] rounded-lg px-3 py-2 focus-within:border-cyan-500/40 transition-colors">
        ...
        <input
          ...
          type="text"
          value={query}
          placeholder="Tìm máy bay, người hoặc tàu..."
      ...
      <AnimatePresence>
        {isOpen && filtered.length > 0 && (
          ...
          >
            ...
            <div className="px-3 py-1.5 border-t border-[var(--border-primary)] bg-[var(--bg-primary)]/50 text-[8px] text-[var(--text-muted)] font-mono tracking-widest">
              {filtered.length} KẾT QUẢ — BẤM ĐỂ ĐỊNH VỊ
            </div>
          </motion.div>
        )}
        {isOpen && query.trim() && filtered.length === 0 && (
          ...
          >
            <div className="text-[9px] text-[var(--text-muted)] font-mono tracking-widest">KHÔNG TÌM THẤY KẾT QUẢ</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
