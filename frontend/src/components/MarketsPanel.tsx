    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-[var(--text-muted)] font-mono tracking-widest">THỊ TRƯỜNG TOÀN CẦU</span>
        </div>
          ...
          >
            <div className="border-b border-[var(--border-primary)] pb-3">
              <h2 className="text-xs font-bold tracking-widest text-cyan-400 flex items-center gap-2 mb-2">
                <TrendingUp className="text-cyan-500" size={14} /> MÃ CỔ PHIẾU QUỐC PHÒNG
              </h2>
              <div className="mt-3 flex flex-col gap-2">
                {Object.entries(stocks).map(([ticker, info]: [string, any]) => (
                  <div
                    key={ticker}
                    className="flex items-center justify-between border border-cyan-500/10 bg-cyan-950/10 p-1.5 rounded-sm relative group overflow-hidden"
                  >
                    ...
                    <div className="flex items-center gap-3 text-right z-10">
                      ...
                      <span
                        className={`flex items-center gap-0.5 w-12 justify-end text-[9px] ${info.up ? "text-cyan-400" : "text-red-400"}`}
                      >
            ...
            <div>
              <h2 className="text-xs font-bold tracking-widest text-cyan-400 flex items-center gap-2 mb-2">
                <Droplet className="text-cyan-500" size={14} /> CHỈ SỐ HÀNG HÓA
              </h2>
              <div className="mt-2 flex flex-col gap-2">
                {Object.entries(oil).map(([name, info]: [string, any]) => (
                  <div
                    key={name}
                    className="flex flex-col border border-cyan-500/10 bg-cyan-950/10 p-1.5 rounded-sm justify-between"
                  >
                    <span className="font-bold text-cyan-500 text-[9px] uppercase mb-0.5">{name}</span>
                    <div className="flex items-center justify-between">
                      <span className="text-[var(--text-primary)] font-bold text-[11px]">
                        ${(info.price ?? 0).toFixed(2)}
                      </span>
                      <span
                        className={`flex items-center gap-0.5 text-[9px] ${info.up ? "text-cyan-400" : "text-red-400"}`}
                      >
                        {info.up ? <ArrowUpRight size={10} /> : <ArrowDownRight size={10} />}
                        {Math.abs(info.change_percent ?? 0).toFixed(2)}%
                      </span>
