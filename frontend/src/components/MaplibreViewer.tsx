...
const MaplibreViewer = ({
  ...
  const [vn12nmBoundary, setVn12nmBoundary] = useState<any>(null);
  const [vnLandEez, setVnLandEez] = useState<any>(null);
  const [vnLandEezBoundary, setVnLandEezBoundary] = useState<any>(null);
  const [vnGeojson, setVnGeojson] = useState<any>(null);

  useEffect(() => {
    let cancelled = false;
    fetch(`${API_BASE}/api/boundaries/vn-12nm`)
      .then((r) => r.json())
      .then((gj) => {
        if (!cancelled) setVn12nmBoundary(gj);
      })
      .catch(() => {
        if (!cancelled) setVn12nmBoundary(null);
      });

    fetch(`${API_BASE}/api/boundaries/vn-land-eez`)
      .then((r) => r.json())
      .then((gj) => {
        if (!cancelled) setVnLandEez(gj);
      })
      .catch(() => {
        if (!cancelled) setVnLandEez(null);
      });

    fetch(`${API_BASE}/api/boundaries/vn-land-eez-boundary`)
      .then((r) => r.json())
      .then((gj) => {
        if (!cancelled) setVnLandEezBoundary(gj);
      })
      .catch(() => {
        if (!cancelled) setVnLandEezBoundary(null);
      });

    fetch(`${API_BASE}/api/boundaries/vn-geojson`)
      .then((r) => r.json())
      .then((gj) => {
        if (!cancelled) setVnGeojson(gj);
      })
      .catch(() => {
        if (!cancelled) setVnGeojson(null);
      });
  ...
  useEffect(() => {
    ...
    if (selectedEntity && data) {
      ...
      if (selectedEntity.type === "flight")
        entity = data?.commercial_flights?.find((f: any) => f.icao24 === selectedEntity.id);
      else if (selectedEntity.type === "private_flight")
        entity = data?.private_flights?.find((f: any) => f.icao24 === selectedEntity.id);
      else if (selectedEntity.type === "military_flight")
        entity = data?.military_flights?.find((f: any) => f.icao24 === selectedEntity.id);
      else if (selectedEntity.type === "private_jet")
        entity = data?.private_jets?.find((f: any) => f.icao24 === selectedEntity.id);
      else if (selectedEntity.type === "tracked_flight")
        entity = data?.tracked_flights?.find((f: any) => f.icao24 === selectedEntity.id);
  ...
  const flightHelpers = useMemo(
    () => ({ interpFlight, inView, trackedIcaoSet }),
    [interpFlight, inView, trackedIcaoSet],
  );
  const commConfig: FlightLayerConfig = {
  ...
  const privConfig: FlightLayerConfig = {
    colorMap: COLOR_MAP_PRIVATE,
    groundedMap: GROUNDED_ICON_MAP,
    typeLabel: "private_flight",
    idPrefix: "pflight-",
  };
  const jetsConfig: FlightLayerConfig = {
    colorMap: COLOR_MAP_JETS,
    groundedMap: GROUNDED_ICON_MAP,
    typeLabel: "private_jet",
    idPrefix: "pjet-",
  };
  const milConfig: FlightLayerConfig = {
  ...
  const newsForMap = useMemo(() => {
    if (!data?.news) return [];

    // When zoomed out (global view), reduce density by grouping to 1 marker per country.
    // When zoomed in, keep existing province-level (VN) and coordinate-level behavior.
    const zoom = viewState.zoom ?? 0;
    const COUNTRY_GROUP_ZOOM_THRESHOLD = 4;
    if (zoom >= COUNTRY_GROUP_ZOOM_THRESHOLD) return data.news;

    const byCountry = new globalThis.Map<string, any>();
    for (const item of data.news as any[]) {
      ...
      // Prefer higher risk marker and aggregate articles/counts.
      const prevScore = prev?.risk_score || 0;
      const nextScore = item?.risk_score || 0;
      if (nextScore > prevScore) {
        // Keep previous aggregates but swap headline/coords to the highest-risk representative.
        const mergedArticles = [...(prev.articles || []), ...(item.articles || [])];
        const mergedCount = (prev.cluster_count || 1) + (item.cluster_count || 1);
        byCountry.set(country, {
          ...item,
          articles: mergedArticles,
          cluster_count: mergedCount,
        });
      } else {
        prev.articles = [...(prev.articles || []), ...(item.articles || [])];
        prev.cluster_count = (prev.cluster_count || 1) + (item.cluster_count || 1);
  ...
  const spreadAlerts = useMemo(() => {
    if (!newsForMap) return [];
    return spreadAlertItems(newsForMap as any, viewState.zoom, dismissedAlerts);
  }, [newsForMap, Math.round(viewState.zoom), dismissedAlerts]);
  ...
  const trackedFlightsGeoJSON = useMemo(() => {
    ...
    for (let i = 0; i < data.tracked_flights.length; i++) {
      ...
      const iconId = isPotus
        ? potusIcon
        : grounded
          ? GROUNDED_ICON_MAP[acType]
          : trackedIconMap[acType]?.[alertColor] || trackedIconMap.airliner[alertColor] || "svgAirlinerWhite";
  ...
  const opacityFilter: any = selectedEntity
    ? [
        "case",
        ["all", ["==", ["get", "type"], selectedEntity.type], ["==", ["get", "id"], selectedEntity.id]],
        1.0,
        0.0,
      ]
    : 1.0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {activeLayers.day_night && nightGeoJSON && (
          <Source id="night-overlay" type="geojson" data={nightGeoJSON as any}>
            <Layer
              ...
              type="fill"
              paint={{
                "fill-color": theme === "light" ? "rgba(0,0,0,0.25)" : "rgba(0,0,0,0.45)",
              }}
            />
          </Source>
        )}
        ...
        {vn12nmBoundary && (
          <Source id="vn-12nm-boundary" type="geojson" data={vn12nmBoundary as any}>
            <Layer
              id="vn-12nm-boundary-layer"
              type="line"
              paint={{
                "line-color": "#22c55e",
                "line-width": 1.5,
        ...
        {vnGeojson && (
          <Source id="vn-geojson" type="geojson" data={vnGeojson as any}>
            <Layer
              id="vn-geojson-fill"
              type="fill"
              paint={{
        ...
        {!vnGeojson && vnLandEez && (
          <Source id="vn-land-eez" type="geojson" data={vnLandEez as any}>
            <Layer
              id="vn-land-eez-fill"
              type="fill"
              paint={{
                "fill-color": "rgba(34,197,94,0.10)",
                "fill-opacity": 1,
              }}
            />
          </Source>
        ...
        {!vnGeojson && vnLandEezBoundary && (
          <Source id="vn-land-eez-boundary" type="geojson" data={vnLandEezBoundary as any}>
            <Layer
              id="vn-land-eez-boundary-layer"
              type="line"
              paint={{
                "line-color": "#16a34a",
                "line-width": 2,
          ...
          >
            ...
            <Layer
              ...
              paint={{
                ...
                "circle-radius": ["step", ["get", "point_count"], 12, 10, 15, 100, 20, 1000, 25, 5000, 30],
        ...
        {activeRouteGeoJSON && (
          <Source id="active-route" type="geojson" data={activeRouteGeoJSON as any}>
            <Layer
              ...
              paint={{
                "line-color": ["match", ["get", "type"], "route-origin", "#38bdf8", "route-dest", "#fcd34d", "#ffffff"],
                "line-width": 2,
                "line-dasharray": [2, 2],
                "line-opacity": 0.8,
              }}
        ...
        <Source id="tracked-flights" type="geojson" data={EMPTY_FC as any}>
          ...
          <Layer
            ...
            type="circle"
            filter={["any", ["==", ["get", "iconId"], "svgPotusPlane"], ["==", ["get", "iconId"], "svgPotusHeli"]]}
            paint={{
              "circle-radius": 18,
        ...
        {uavGeoJSON && !selectedEntity && data?.uavs && <UavLabels uavs={data.uavs} inView={inView} />}
        ...
        {dataCentersGeoJSON && (
          <Source
            id="datacenters"
            type="geojson"
            data={dataCentersGeoJSON as any}
            cluster={true}
            clusterRadius={30}
            clusterMaxZoom={8}
          >
            ...
            return (
              ...
              >
                <div className="map-popup border border-cyan-500/30">
                  <div className="map-popup-title text-[#00c8ff]">🛰️ {sat.name}</div>
        ...
        {selectedEntity?.type === "kiwisdr" &&
          (() => {
            const receiver = data?.kiwisdr?.find(
              (k: any) => k.name === selectedEntity.name || String(k.id) === String(selectedEntity.id),
            );
              ...
              >
                <div className="map-popup !border-amber-500/40" style={{ borderWidth: 1, borderStyle: "solid" }}>
                  ...
                  <div className="map-popup-subtitle text-amber-600/80 border-b border-amber-900/30 pb-1 flex items-center gap-1.5">
                    <Radio size={10} /> THU TÍN HIỆU MẠNG CÔNG KHAI
                  </div>

                  {props.location && (
                    <div className="map-popup-row mt-1">
                      Vị trí: <span className="text-white">{props.location}</span>
                    </div>
                  )}
                  {props.users !== undefined && (
                    <div className="map-popup-row">
                      Người dùng hoạt động:{" "}
                      <span className={props.users >= (props.users_max || 4) ? "text-red-400" : "text-amber-400"}>
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-2 border-t border-[var(--border-primary)]">
                    ...
                    >
                      <Activity size={10} /> THEO DÕI TÍN HIỆU
                    </button>
                    ...
                    {props.url && (
                      <a
                        ...
                        title="Mở giao diện SDR ở tab mới"
                      >
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    <div
                      ...
                      style={{ color: ship.yacht_alert ? "#FF69B4" : ship.type === "carrier" ? "#ffaa00" : "#3b82f6" }}
                    >
                  ...
                  {ship.estimated && (
                    <div className="map-popup-subtitle text-[#ff6644] border-b border-[#ff664450] pb-1">
                      VỊ TRÍ ƯỚC TÍNH — {ship.source || "OSINT DERIVED"}
                    </div>
                  )}
                  {ship.type && (
                    <div className="map-popup-row">
                      Loại: <span className="text-white capitalize">{ship.type.replace("_", " ")}</span>
                    </div>
                  )}
                  {ship.mmsi && (
                    <div className="map-popup-row">
                  ...
                  {ship.country && (
                    <div className="map-popup-row">
                      Cờ: <span className="text-white">{ship.country}</span>
                    </div>
                  )}
                  {ship.destination && (
                    <div className="map-popup-row">
                      Điểm đến: <span className="text-[#44ff88]">{ship.destination}</span>
                    </div>
                  )}
                  {typeof ship.sog === "number" && ship.sog > 0 && (
                    <div className="map-popup-row">
                      Tốc độ: <span className="text-[#00e5ff]">{ship.sog.toFixed(1)} kn</span>
                    </div>
                  )}
                  <div className="map-popup-row">
                    Hướng:{" "}
                  ...
                  {ship.type === "carrier" && ship.source && (
                    <div className="mt-1.5 p-[5px_7px] bg-[rgba(255,170,0,0.08)] border border-[rgba(255,170,0,0.3)] rounded text-[9px] tracking-wide">
                      ...
                      {ship.desc && <div className="text-[#aaa] mt-0.5 text-[8px] leading-tight">{ship.desc}</div>}
                    </div>
                  )}
                  {ship.type !== "carrier" && ship.last_osint_update && (
                    <div className="map-popup-row">
                      Cập nhật OSINT gần nhất:{" "}
                      <span className="text-[#888]">{new Date(ship.last_osint_update).toLocaleDateString()}</span>
                    </div>
                  )}
                  {ship.yacht_alert && (
                    <div className="mt-1.5 p-[5px_7px] bg-[rgba(255,105,180,0.08)] border border-[rgba(255,105,180,0.3)] rounded text-[9px] tracking-wide">
                      <div className="text-[#FF69B4] font-bold mb-0.5">DU THUYỀN THEO DÕI</div>
                      <div>
                        Chủ sở hữu: <span className="text-white">{ship.yacht_owner}</span>
                      </div>
                      {ship.yacht_builder && (
                        <div>
                          Nhà sản xuất: <span className="text-[#888]">{ship.yacht_builder}</span>
                        </div>
                      )}
                      {(ship.yacht_length ?? 0) > 0 && (
                        <div>
                          Chiều dài: <span className="text-[#888]">{ship.yacht_length}m</span>
                        </div>
                      )}
                      {(ship.yacht_year ?? 0) > 0 && (
                        <div>
                          Năm: <span className="text-[#888]">{ship.yacht_year}</span>
                        </div>
                      )}
                      {ship.yacht_category && (
                        <div>
                          Phân loại: <span className="text-[#FF69B4]">{ship.yacht_category}</span>
                        </div>
                      )}
                      {ship.yacht_link && (
                        <a
        ...
        {selectedEntity?.type === "datacenter" &&
          (() => {
            ...
            const outagesInCountry = (data?.internet_outages || []).filter(
              (o: any) => o.country_name && dc.country && o.country_name.toLowerCase() === dc.country.toLowerCase(),
            );
            return (
              <Popup
                longitude={dc.lng}
              ...
              >
                <div className="map-popup bg-[#1a1035] border border-violet-400/40 text-[#e9d5ff] min-w-[200px]">
                  <div className="map-popup-title text-violet-400 border-b border-violet-400/20 pb-1">{dc.name}</div>
                  {dc.company && (
                    <div className="map-popup-row">
                  ...
                  {!dc.city && dc.country && (
                    <div className="map-popup-row">
                      Quốc gia: <span className="text-white">{dc.country}</span>
                    </div>
                  )}
                  {outagesInCountry.length > 0 && (
                    <div className="mt-1.5 px-2 py-1 bg-red-500/15 border border-red-400/40 rounded text-[10px] text-[#ff6b6b]">
                      MẤT KẾT NỐI TRONG KHU VỰC —{" "}
                      {outagesInCountry.map((o: any) => `${o.region_name} (${o.severity}%)`).join(", ")}
                    </div>
                  )}
                  <div className="mt-1.5 text-[9px] text-violet-600 tracking-wider">TRUNG TÂM DỮ LIỆU</div>
                </div>
              </Popup>
            );
          })()}
        ...
        {(() => {
          ...
          const item = data.gdelt.find(
            (g: any) => (g.properties?.name || String(g.geometry?.coordinates)) === selectedEntity.id,
          );
              ...
              <div className="bg-[var(--bg-secondary)]/90 backdrop-blur-md border border-orange-800 rounded-lg flex flex-col z-[100] font-mono shadow-[0_4px_30px_rgba(255,140,0,0.4)] pointer-events-auto overflow-hidden w-[300px]">
                <div className="p-2 border-b border-orange-500/30 bg-orange-950/40 flex justify-between items-center">
                  <h2 className="text-[10px] tracking-widest font-bold text-orange-400 flex items-center gap-1">
                    <AlertTriangle size={12} className="text-orange-400" /> TIN TỨC TẠI HIỆN TRƯỜNG
                  </h2>
                  <button
                    onClick={() => onEntityClick?.(null)}
                  ...
                  <div className="flex flex-col gap-1 mt-1">
                    ...
                    <div className="flex flex-col gap-2 max-h-[200px] overflow-y-auto styled-scrollbar mt-1">
                      {(() => {
                        ...
                        if (urls.length === 0)
                          return <span className="text-[var(--text-muted)] text-[10px]">Không có bài viết.</span>;
        ...
        {selectedEntity?.type === "liveuamap" &&
          data?.liveuamap?.find((l: any) => String(l.id) === String(selectedEntity.id)) &&
          (() => {
                ...
                <div className="bg-[var(--bg-secondary)]/90 backdrop-blur-md border border-yellow-800 rounded-lg flex flex-col z-[100] font-mono shadow-[0_4px_30px_rgba(255,255,0,0.3)] pointer-events-auto overflow-hidden w-[280px]">
                  <div className="p-2 border-b border-yellow-500/30 bg-yellow-950/40 flex justify-between items-center">
                    <h2 className="text-[10px] tracking-widest font-bold text-yellow-400 flex items-center gap-1">
                      <AlertTriangle size={12} className="text-yellow-400" /> SỰ KIỆN TÁC CHIẾN KHU VỰC
                    </h2>
                    <button
                      onClick={() => onEntityClick?.(null)}
                      className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
                  ...
                  <div className="p-3 flex flex-col gap-2">
                    ...
                    <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-1 mt-1">
                      <span className="text-[var(--text-muted)] text-[9px]">THỜI GIAN</span>
        ...
        {(() => {
          ...
          return (
            ...
            >
              <div
                className={`bg-[var(--bg-secondary)]/90 backdrop-blur-md border ${borderColor} rounded-lg flex flex-col z-[100] font-mono shadow-[0_4px_30px_${shadowColor}] pointer-events-auto overflow-hidden w-[280px]`}
              >
                <div className={`p-2 border-b ${borderColor}/50 ${bgHeaderColor} flex justify-between items-center`}>
                  <h2 className={`text-[10px] tracking-widest font-bold ${threatColor} flex items-center gap-1`}>
                    <AlertTriangle size={12} className={threatColor} /> CHẶN NGUY CƠ
                  </h2>
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] ${threatColor} font-mono font-bold animate-pulse`}>
                ...
                <div className="p-3 flex flex-col gap-2">
                  ...
                  <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-1 mt-1">
                    <span className="text-[var(--text-muted)] text-[9px]">NGUỒN</span>
                  ...
                  {item.link && (
                    <div className="flex justify-between items-center mt-1">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className={`${threatColor} hover:text-red-300 text-[9px] font-bold underline`}
                      >
                        Xem chi tiết
                      </a>
                    </div>
          ...
          >
            <div className="flex flex-col items-center pointer-events-none">
              ...
              <div className="mt-2 bg-black/80 border border-emerald-800 rounded px-2 py-1 text-[9px] font-mono text-emerald-400 tracking-widest whitespace-nowrap shadow-[0_0_10px_rgba(16,185,129,0.3)]">
                {regionDossierLoading ? "ĐANG TỔNG HỢP..." : "▶ MỤC TIÊU TÌNH BÁO"}
              </div>
            </div>
          </Marker>
        )}
        ...
        {measurePoints &&
          measurePoints.map((pt: any, idx: number) => (
