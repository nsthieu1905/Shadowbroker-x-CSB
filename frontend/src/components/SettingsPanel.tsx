...
const WEIGHT_LABELS: Record<number, string> = { 1: "THẤP", 2: "TB", 3: "CHUẨN", 4: "CAO", 5: "NGUY CẤP" };
  ...
  const saveFeeds = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setFeedsDirty(false);
        try {
          await fetch(`${API_BASE}/api/refresh/news`, {
            method: "POST",
            headers: adminHeaders(),
          });
          setFeedMsg({ type: "ok", text: "Đã lưu nguồn. Đã xếp hàng làm mới tin." });
        } catch {
          setFeedMsg({
            type: "ok",
            text: "Đã lưu nguồn. (Không gọi được làm mới tin — thay đổi sẽ áp dụng ở lần làm mới theo lịch tiếp theo.)",
          });
        }
      } else {
        const d = await res.json().catch(() => ({}));
        setFeedMsg({ type: "err", text: d.message || "Lưu thất bại" });
      }
    } catch (err) {
      console.error("Failed to save feeds", err);
      setFeedMsg({ type: "err", text: "Lỗi mạng" });
    } finally {
      setFeedSaving(false);
    }
  };
  ...
  const resetFeeds = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setFeeds(d.feeds || []);
        setFeedsDirty(false);
        setFeedMsg({ type: "ok", text: "Đã khôi phục mặc định" });
      }
    } catch (err) {
      console.error("Failed to reset feeds", err);
      setFeedMsg({ type: "err", text: "Khôi phục thất bại" });
    }
  };
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[var(--border-primary)]/40 bg-[var(--bg-primary)]/30">
              ...
              <input
                ...
                placeholder="Nhập khóa quản trị để thao tác chức năng được bảo vệ..."
              ...
              />
              {adminKey && <span className="text-[8px] font-mono text-green-400/70 tracking-widest">ĐÃ ĐẶT</span>}
            </div>
              ...
              <>
                ...
                <div className="mx-4 mt-4 p-3 rounded-lg border border-cyan-900/30 bg-cyan-950/10">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-[10px] text-[var(--text-secondary)] font-mono leading-relaxed">
                      Khóa API được lưu cục bộ trong file <span className="text-cyan-400">.env</span> của backend. Các
                      khóa có dấu <Key size={8} className="inline text-yellow-500" /> là bắt buộc để dùng đầy đủ tính
                      năng. API công khai không cần khóa.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto styled-scrollbar p-4 space-y-3">
                  {Object.entries(grouped).map(([category, categoryApis]) => {
                    ...
                    return (
                      <div
                        key={category}
                        className="rounded-lg border border-[var(--border-primary)]/60 overflow-hidden"
                      >
                        <button
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-[9px] font-mono tracking-widest font-bold px-2 py-0.5 rounded border ${colorClass}`}
                            >
                              {category.toUpperCase()}
                            </span>
                            <span className="text-[10px] text-[var(--text-muted)] font-mono">
                              {categoryApis.length} {categoryApis.length === 1 ? "dịch vụ" : "dịch vụ"}
                            </span>
                          </div>
                          {isExpanded ? (
                                        ...
                                        <div className="flex items-center gap-1.5">
                                          ...
                                          >
                                            <span className="text-[var(--text-muted)] tracking-wider">
                                              {api.is_set ? api.value_obfuscated : "Bấm để đặt khóa..."}
                                            </span>
                                          </div>
                                        </div>
                                      )}
                ...
                <div className="p-4 border-t border-[var(--border-primary)]/80">
                  <div className="flex items-center justify-between text-[9px] text-[var(--text-muted)] font-mono">
                    <span>{apis.length} API ĐÃ ĐĂNG KÝ</span>
                    <span>{apis.filter((a) => a.has_key).length} KHÓA ĐÃ CẤU HÌNH</span>
                  </div>
                </div>
              </>
            )}
              ...
              <>
                ...
                <div className="mx-4 mt-4 p-3 rounded-lg border border-orange-900/30 bg-orange-950/10">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-[10px] text-[var(--text-secondary)] font-mono leading-relaxed">
                      Cấu hình nguồn RSS/Atom cho bảng tin tình báo. Mỗi nguồn được chấm điểm theo từ khóa và được ưu
                      tiên theo mức bạn đặt. Tối đa <span className="text-orange-400">{MAX_FEEDS}</span> nguồn.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto styled-scrollbar p-4 space-y-2">
                  {feeds.map((feed, idx) => (
                    <div
                      key={idx}
                      className="rounded-lg border border-[var(--border-primary)]/60 p-3 hover:border-[var(--border-secondary)]/60 transition-colors group"
                    >
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          >
                            {WEIGHT_LABELS[feed.weight] || "CHUẨN"}
                          </span>
                        </div>
                        <button
                  ...
                  >
                    <Plus size={10} />
                    THÊM NGUỒN ({feeds.length}/{MAX_FEEDS})
                  </button>
                </div>
                ...
                {feedMsg && (
                  <div
                    className={`mx-4 mb-2 px-3 py-2 rounded text-[10px] font-mono ${feedMsg.type === "ok" ? "text-green-400 bg-green-950/20 border border-green-900/30" : "text-red-400 bg-red-950/20 border border-red-900/30"}`}
                  >
                    {feedMsg.text}
                  </div>
                )}
                ...
                <div className="p-4 border-t border-[var(--border-primary)]/80">
                  ...
                  <div className="flex items-center justify-between text-[9px] text-[var(--text-muted)] font-mono mt-2">
                    <span>
                      {feeds.length}/{MAX_FEEDS} NGUỒN
                    </span>
                    <span>ƯU TIÊN: 1=THẤP 5=NGUY CẤP</span>
                  </div>
                </div>
              </>
            )}
